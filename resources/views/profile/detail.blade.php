                ...
                <form method="post" action="{{route('storeProfile')}}" enctype="multipart/form-data">
                    <div class="row mb-4">
                        <div class="col-6 d-flex flex-row ">
                            ...
                            <input type="date" name="date" value="{{date_format(date_create($profile->created_at),"Y-m-d")}}" class="form-control " required>
                        </div>
                        <div class="col-6 d-flex flex-row ">
                            <label for=" " class="w-50 mt-1">Service Date: </label>
                            <input type="date" value="{{date_format(date_create($profile->updated_at),"Y-m-d")}}" name="serviceDate" class="form-control " required>
                        </div>
                    </div>
                    <div class="row mb-4">
                        <div class="col-6 d-flex flex-row ">
                            <label for=" " class="w-50 mt-1">Num Affair: </label>
                            <input type="text" value="{{$profile->num_affair}}" name="numAffair" class="form-control " required>
                        </div>
                        <div class="col-6 d-flex flex-row ">
                            <label for=" " class="w-50 mt-1">Service price: </label>
                            <input type="text" value="{{$profile->service_price}}" name="servicePrice" class="form-control " required>
                        </div>
                    </div>
                    <div class="row mt-5 ">
                        <div class="col-md py-1 d-flex flex-row">
                            <label for=" " class="col-md-2   ">Default jurisdiction:</label>
                            <input type="text " value="{{$profile->default}}" name="default" class="col-md-3 form-control w-50" required>
                        </div>
                    </div>
                    @include('error')
                    @csrf
                ...
                <table class="table table-hover"   style="width: 90%;margin: 2% 5%;">
                    ...
                    @foreach($result as $id => $value)
                        <tr class="d-flex flex-row justify-content-between">
                            ...
                            <td class="col-2">
                                <form action="{{route('showImage',['profile_id' => $profile->id,'id'=>$value->id])}}" method="get">
